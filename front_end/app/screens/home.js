...
export default function Home () {
    ...
    function toWallet () {
        fetch("http://192.168.8.36:8080/user/userWallet").then(res => {
            return res.json()
        }).then(info => {
            //console.log(info)
            var x = info.map(i => {
            ...
            if(x.filter(Boolean).length > 0){
                ...
                fetch(`http://192.168.8.36:8080/user/add/${user.uid}`, {
                        method : 'POST',
                        headers : {
                ...
                {names.map((i) => (
                    <TouchableOpacity onPress={() => history.push('/Details', i)} key={i.id}>
                        <View style={styles.listItem}>
                            <Image source={{uri: `https://assets.coincap.io/assets/icons/${i.symbol.toLowerCase()}@2x.png`}}  style={{width:20, height:20,borderRadius:30}} />
